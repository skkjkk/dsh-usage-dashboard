    ...
    function CalendarChart(props) {
      ...
      const colsEl = Array.from({ length: COLS }, (_, c) => {
        const cells = Array.from({ length: 7 }, (_, r) => {
          ...
          const d = new Date(t)
          const tip = d.getFullYear() + '年' + (d.getMonth() + 1) + '月' + d.getDate() + '日：' + fmtIntl(tokens) + ' tokens'
          return h('div', {
            key: r,
    ...
    function RecordsCard(props) {
      ...
      React.useEffect(() => {
        ...
        fetchPage(1).then((r) => {
          ...
          setLoading(false)
        }).catch(() => { if (alive) { setLoading(false); setData({ rows: [], total: 0, gran: 'day' }) } })
        return () => { alive = false }
      ...
      const go = (p) => {
        ...
        fetchPage(p).then((r) => {
          setData({ rows: (r && r.rows) ? r.rows : [], total: (r && r.total) || 0, gran: (r && r.gran) || 'day' })
          setLoading(false)
        }).catch(() => { setLoading(false); setData((d) => d || { rows: [], total: 0, gran: 'day' }) })
      }
