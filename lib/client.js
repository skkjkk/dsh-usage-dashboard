  ...
  apply(ctx) {
    ...
    function CalendarChart(props) {
      ...
      const dayAt = (offset) => {
        ...
        return gridStart + offset * 86400000
    ...
    function Dashboard() {
      ...
      const initialCustom = prefs.custom || defaultCustom()
      const [custom, setCustom] = React.useState(initialCustom)
      const [appliedCustom, setAppliedCustom] = React.useState(initialCustom)
      ...
      const load = React.useCallback((silent) => {
        ...
        const queryCustom = appliedCustom
        const showBusy = !silent || applyPending.current
        if (showBusy) setBusy(true)
        host.call('usage', { range: range, from: queryCustom.from, to: queryCustom.to, models: modelSel && modelSel.length ? modelSel : null, projects: projectSel ? [projectSel] : null }).then((r) => {
          ...
          const key = range + '|' + queryCustom.from + '|' + queryCustom.to + '|' + (modelSel || []).join(',') + '|' + (projectSel || '')
      ...
      }, [range, appliedCustom, modelSel, projectSel])
      React.useEffect(() => {
      ...
      return h('div', { className: 'dd-root' },
        ...
        h('div', { className: 'dd-dash' },
          h(FilterBar, {
            ...
            meta: meta, busy: busy,
            onApply: () => { applyPending.current = true; setAppliedCustom(custom) }
          }),
          h('div', { className: 'dd-anim' + (anim ? ' on' : '') },
            ...
            h(RecordsCard, { range: range, custom: appliedCustom, modelSel: modelSel, projectSel: projectSel, costMode: costMode }))))
    }
