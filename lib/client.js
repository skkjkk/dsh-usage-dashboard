window.__ModuleLoader__.load({
  id: "@linxin666/dsh-usage-dashboard",
  factory: (require) => {
    var module = { exports: {} };
...
module.exports = {
  ...
  apply(ctx) {
    ...
    ctx.effect(() => {
        const tagId = "@linxin666/dsh-usage-dashboard/dashboard.module.css";
        if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css=' + JSON.stringify(tagId) + ']') === null) {
          const tag = document.createElement('style');
          tag.dataset.plugin = "@linxin666/dsh-usage-dashboard";
    ...
    const PALETTE = ['#18181b', '#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#ec4899', '#06b6d4', '#84cc16', '#71717a', '#d4d4d8']
    // 活跃度日历 7 阶色阶（灰 → 深蓝绿）与每日 token 阈值：0 | 1 | 50万 | 500万 | 2000万 | 6000万 | 1.5亿
    const CAL_LEVELS = [
      { min: 0, color: '#EAEAEA' },
      { min: 1, color: '#C9EAD3' },
      { min: 500000, color: '#8AD8B3' },
      { min: 5000000, color: '#58C897' },
      { min: 20000000, color: '#30B07C' },
      { min: 60000000, color: '#1A8E62' },
      { min: 150000000, color: '#106345' }
    ]
    ...
    function DistributionCard(props) {
      ...
      const items = raw.map((x) => ({ id: x.id, label: x.label || x.id, tokens: x.tokens, cost: x.cost, v: mode === 'cost' ? x.cost : x.tokens }))
        .filter((x) => x.v > 0)
        .sort((a, b) => b.v - a.v)
      const total = items.reduce((s, x) => s + x.v, 0)
      ...
      const head = h('div', { className: 'dd-chart-head' },
        h('div', { className: 'dd-chart-title' },
          h('span', { className: 'icon' }, ICON_PIE),
          h('span', null, props.title)),
        h('div', { className: 'dd-chart-tools' },
      ...
      if (items.length === 0) {
        ...
        body = h('div', { className: 'dd-dist-body' },
          h('div', { className: 'dd-dist-donut' },
            h(Donut, { items: items.map((x, i) => ({ v: x.v, color: PALETTE[i % PALETTE.length] })) }),
          ...
          h('div', { className: 'dd-dist-legend' },
            items.map((x, i) => h('div', { key: x.id, className: 'dd-dist-item' },
              h('span', { className: 'dot', style: { backgroundColor: PALETTE[i % PALETTE.length] } }),
    ...
    // GitHub 风格活跃度日历：53 周 × 7 天网格（周日起始），顶部月份标签，7 阶色阶 + 右下角图例
    function CalendarChart(props) {
      ...
      const dayLabels = DAY_NAMES.map((name, r) => h('span', { key: r }, (r === 1 || r === 3 || r === 5) ? name.replace('周', '') : ''))
      return h('div', { className: 'dd-chart' },
        head,
        h('div', { className: 'dd-cal' },
          ...
          h('div', { className: 'dd-cal-body' },
            h('div', { className: 'dd-cal-days' }, dayLabels),
            h('div', { className: 'dd-cal-cols' }, colsEl)),
          h('div', { className: 'dd-cal-legend' },
            h('span', { className: 'lbl' }, '少'),
            h('div', { className: 'dots' }, CAL_LEVELS.map((l, i) => h('span', { key: i, className: 'dd-cal-dot', style: { backgroundColor: l.color } }))),
          ...
          h('div', { className: 'dd-records-scroll' },
            h('table', null,
              h('colgroup', null,
                ...
                h('col', { style: { width: 64 } }),
                h('col', { style: { width: 64 } }),
                h('col', { style: { width: 84 } }),
                h('col', { style: { width: 72 } })),
              h('thead', null, h('tr', null,
                h('th', { className: 'num' }, '时间'),
                h('th', null, '会话'),
                h('th', null, '模型'),
                h('th', null, '工具'),
                ...
                h('th', { className: 'num' }, '时长'))),
              h('tbody', null, rows.map((r) => h('tr', { key: r.t + ':' + (r.models || []).join(',') },
                h('td', { className: 'num' }, fmtBucket(r.t, gran)),
                h('td', null, h('span', { className: 'sess', title: r.sessions + ' 个会话' }, r.sessions)),
                h('td', null, h('span', { className: 'sess', title: (r.models || []).join(', ') }, (r.models || []).join(', ') || '-')),
                h('td', null, 'dsh'),
                ...
                h('td', { className: 'num' }, fmtDur(r.dur / 1000))))))),
    ...
    function Dashboard() {
      ...
      React.useEffect(() => {
        load()
        const off = setInterval(() => load(), 60000)
      ...
      return h('div', { className: 'dd-dash' },
        ...
        h('div', { className: 'dd-dist-row' },
          h(DistributionCard, { title: '模型分布', items: (meta.dist && meta.dist.models) || [] }),
          h(DistributionCard, { title: '项目分布', items: (meta.dist && meta.dist.projects) || [] })),
