...
export function apply(ctx, config) {
  ...
  function ensureState(id, header) {
    ...
    if (!st) {
      st = { rollup: emptyRollup(), cwd: (header && header.cwd) || '', title: null, at: Date.now(), pending: [], needsReload: false, loadingPromise: null, listed: false, lastEventEpoch: 0, lastListedEventEpoch: 0, missingListEpoch: null }
      states.set(id, st)
    } else {
  ...
  async function getRollups() {
    ...
    initPromise = (async () => {
      ...
      for (const id of Array.from(states.keys())) {
        const st = states.get(id)
        if (!seen.has(id) && !keepMissingState(st, listEpoch)) states.delete(id)
      }
      ready = true
      return currentRollups()
  ...
  ctx.on('session/event', (session, event) => {
    ...
    let st = states.get(id)
    if (st) st.lastEventEpoch = eventEpochNow
    if (!st) {
